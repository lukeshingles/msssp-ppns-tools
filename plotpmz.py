...
for i in range(0, len(convectiveBoundaries), 2):
    ...
    plt.bar(convectiveBoundaries[i], 1, width=convectiveBoundaries[i+1]-convectiveBoundaries[i], bottom=10**-8.5,
            lw=0, alpha=0.5, color="0.65", edgecolor='black', linewidth=5)

for i in range(len(abundArray)):
    lineColor = ['0', 'r', 'b', 'y'][i % 4]
    lineDashes = [(), (15, 2), (3, 2), (0.2, 6), (4, 2)][i % 4]

    plt.plot(massArray, abundArray[i], color=lineColor, marker='o', markersize=0, lw=3,
             dashes=lineDashes, markeredgewidth=0, label=headerRow[1+i])
